...
export class BeanstalkScene {
  ...
  render() {
    ...
    while (this.branches.length > 0 && this.branches[0].ringIndex < 0) {
      ...
      if (oldBranch.gameCase) {
        ...
        oldBranch.gameCase.releaseTexture();
      ...
      for (let leafIndex = this.branches.length - 1; leafIndex >= 0; leafIndex--) {
        ...
        if (!branch.gameCase || branch.gameIndex >= this.totalCount) {
          ...
          if (branch.gameCase) {
            ...
            branch.gameCase.releaseTexture();
  ...
  _preloadTexturesForVelocity(velocity) {
    ...
    // Throttle: run at most every 500ms instead of every frame
    const now = performance.now();
    if (now - this.lastPreloadTime < 500) {
      return;
    }
    ...
    // Collect URLs to preload — only the primary cover URL per game
    const urls = [];
    for (let index = preloadStart; index <= preloadEnd; index++) {
      const item = this.items[index];
      if (item && item.game) {
        const url = getCoverUrl(item.game);
        if (url) {
          urls.push(url);
        }
      }
